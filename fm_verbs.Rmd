...
```{r}
IsImperativeProps <- plyr::ddply(data_verbs, c("corpus","age_months"), summarise,
...
h + geom_histogram(aes(color =factor(corpus), 
                       fill=factor(corpus)), alpha=0.8) + 
  labs(#title="Relative Action Onset Time by Corpus", 
       #subtitle="Relative to Verb Utterance",
       x="Event Onset (seconds) Relative to Verb Onset", 
       y = "Frequency of Event Occurrence",
       fill="Corpus") + xlim(c(-20, 15)) +  
  scale_fill_discrete(name = "Corpus", labels = c("American", "Japanese"))
```
